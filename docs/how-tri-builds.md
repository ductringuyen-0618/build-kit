...
| 3. Human gate | The proposal is pushed with `status: proposed` and the workflow waits for a decision event. Nothing is built until a human flips it to `approved`. Undecided proposals are nudged at 3 and 6 days and expire at 7. | the workflow's await-approval step; commit `chore(coo): approve 001-...` (b6f336e); agent-os commit 6da2602 for the nudge/expiry rule. |
...
| 5. Validate with the real check commands | A separate run with only a shell tool executes every command in the project's configured check list, reports exit code and the last 40 lines per command, first line exactly `PASS` or `FAIL`. | `feature-validate` skill; the report's Validation section shows both commands with exit codes and output. |
| 6. Independent review | A read-only run (file reading and search tools, no shell) judges the diff against the proposal's Proposed solution and Validation contract, not against generic taste. `FAIL` must name file, problem and what fixed looks like. | `feature-review` skill; the report's Review section cites `tailwind.config.js:3`, `index.html:27-34`, etc. |
| 7. One fix attempt | If validate or review fails, the worker runs again with the verbatim failure text as its prior-failure input, then validate and review run again. A second failure fails the request and hands the proposal back as `approved`. | The workflow's build-fix, validate-fix and review-fix steps and its unmark-building catch. The `fix: resolve lint errors blocking dark mode PR` commit (0cca70d) is this path in practice. |
...
| 9. CI gate | Poll the PR's checks every 30 s for up to 30 min. If any check fails, fetch the failing job's log, run the worker once more with that log as `priorFailure`, push, wait again. A second red fails the request. Only after green does `ship` run. | The workflow's CI polling and ci-failure-log, build-fix-ci and push-fix steps; the COO prompt's rule "nothing is shipped until the pull request's CI is green. Local checks are necessary, never sufficient." |
| 10. Ship with a report | Flip the proposal to `shipped`, write a report file with branch, PR, validation output, review output, and bump the shipped counter in `state.md`. | `chore(coo): ship ...` (91b3ca0), `docs(coo): report ...` (b51bca4); agent-os reports for PR #6 and PR #7. |
...
| `feature-build` | Step 4 and step 7. Resume from `git log`, fix the prior failure first, small commits, never push. | The "never push" rule is what makes the CI gate a separate, checkable step. |
...
7. **The proposal's H1 was written twice** (once by the brief, once by
   the page writer). Cosmetic, but it shows why the brief should own
