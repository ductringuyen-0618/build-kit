...
| [webapp-testing](https://github.com/anthropics/skills/tree/main/skills/webapp-testing) | Browser-driven checks with Playwright; complements `verify-feature`. | `npx skills add anthropics/skills -s webapp-testing` |

Also useful, if the tool has it: a Playwright MCP server for screenshots
(`verify-feature` and `verify-techpulse` use one) and a docs MCP such as
